...
// Position
VertexData amk_snow_vertexData_position;
VertexData amk_snow_vertexData_random;
...
UniformData amk_uniformData_points;

// Prebaked arrays of position and random for points
float amk_points_position[POINTS_COUNT * 3]; // x, y, z for each point
float amk_points_random[POINTS_COUNT];

// Descriptor Set
VkDescriptorSet amk_vkDescriptorSet_points = VK_NULL_HANDLE;
...
VkResult initialize(void) {
    ...
    // Create Texture
    vkResult = createTexture("snow_particle.png");
    if(vkResult != VK_SUCCESS) {
    ...
    vkClearColorValue.float32[0] = 0.17f;
    vkClearColorValue.float32[1] = 0.18f;
    vkClearColorValue.float32[2] = 0.17f;
...
void uninitialize(void){
    ...
    if(vkDescriptorPool) {
        ...
        amk_vkDescriptorSet_points = NULL;
    }
    ...
    if(amk_uniformData_points.vkDeviceMemory) {
        vkFreeMemory(vkDevice, amk_uniformData_points.vkDeviceMemory, NULL);
        fprintf(fptr, "uninitialize(): vkFreeMemory() Succeed for Uniform Buffer Memory!\n");
        amk_uniformData_points.vkDeviceMemory = VK_NULL_HANDLE;
    }

    if(amk_uniformData_points.vkBuffer) {
        vkDestroyBuffer(vkDevice, amk_uniformData_points.vkBuffer, NULL);
        fprintf(fptr, "uninitialize(): vkDestroyBuffer() Succeed for Uniform Buffer!\n");
        amk_uniformData_points.vkBuffer = VK_NULL_HANDLE;
    }
    ...
    if(amk_snow_vertexData_position.vkDeviceMemory) {
        vkFreeMemory(vkDevice, amk_snow_vertexData_position.vkDeviceMemory, NULL);
        fprintf(fptr, "uninitialize(): vkFreeMemory() Succeed for Vertex Buffer for Position!\n");
        amk_snow_vertexData_position.vkDeviceMemory = VK_NULL_HANDLE;
    }

    if(amk_snow_vertexData_position.vkBuffer) {
        vkDestroyBuffer(vkDevice, amk_snow_vertexData_position.vkBuffer, NULL);
        fprintf(fptr, "uninitialize(): vkDestroyBuffer() Succeed for Vertex Buffer for Position!\n");
        amk_snow_vertexData_position.vkBuffer = VK_NULL_HANDLE;
    }

    if(amk_snow_vertexData_random.vkDeviceMemory) {
        vkFreeMemory(vkDevice, amk_snow_vertexData_random.vkDeviceMemory, NULL);
        fprintf(fptr, "uninitialize(): vkFreeMemory() Succeed for Vertex Buffer for Position!\n");
        amk_snow_vertexData_random.vkDeviceMemory = VK_NULL_HANDLE;
    }

    if(amk_snow_vertexData_random.vkBuffer) {
        vkDestroyBuffer(vkDevice, amk_snow_vertexData_random.vkBuffer, NULL);
        fprintf(fptr, "uninitialize(): vkDestroyBuffer() Succeed for Vertex Buffer for Position!\n");
        amk_snow_vertexData_random.vkBuffer = VK_NULL_HANDLE;
    }
    // Destroy  Command Buffers
    if(vkCommandBuffer_array) {
        for(uint32_t i = 0; i < swapchainImageCount; i++) {
...
VkResult createVertexBuffer(void) {
    // variables
    VkResult vkResult = VK_SUCCESS;
    void calculateRandomPositionAndRandom(void);

    calculateRandomPositionAndRandom();

    // VertexData for Triangle Position
    // Step 2
    memset((void*)&amk_snow_vertexData_position, 0, sizeof(VertexData));
    ...
    vkBufferCreateInfo.size = sizeof(amk_points_position);
    vkBufferCreateInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
    
    // Setp 4
    vkResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &amk_snow_vertexData_position.vkBuffer);
    if(vkResult != VK_SUCCESS) {
    ...
    vkGetBufferMemoryRequirements(vkDevice, amk_snow_vertexData_position.vkBuffer, &vkMemoryRequirements);
    ...
    //Setp 9
    vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &amk_snow_vertexData_position.vkDeviceMemory);
    if(vkResult != VK_SUCCESS) {
    ...
    // Step 10
    vkResult = vkBindBufferMemory(vkDevice, amk_snow_vertexData_position.vkBuffer, amk_snow_vertexData_position.vkDeviceMemory, 0);
    if(vkResult != VK_SUCCESS) {
    ...
    vkResult = vkMapMemory(
        vkDevice,
        amk_snow_vertexData_position.vkDeviceMemory,
        0,
    ...
    // Step 12
    memcpy(data, amk_points_position, sizeof(amk_points_position));

    // Step 13
    vkUnmapMemory(vkDevice, amk_snow_vertexData_position.vkDeviceMemory);

    fprintf(fptr, "\n");

    memset((void*)&amk_snow_vertexData_random, 0, sizeof(VertexData));
    ...
    vkBufferCreateInfo.pNext = NULL;
    vkBufferCreateInfo.flags = 0; // No flags, Valid Flags are used in scattered buffer
    vkBufferCreateInfo.size = sizeof(amk_points_random);
    vkBufferCreateInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
    
    // Setp 4
    vkResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &amk_snow_vertexData_random.vkBuffer);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkCreateBuffer() Failed for Position!.\n");
        return (vkResult);
    } else {
    ...
    vkGetBufferMemoryRequirements(vkDevice, amk_snow_vertexData_random.vkBuffer, &vkMemoryRequirements);
    ...
    //Setp 9
    vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &amk_snow_vertexData_random.vkDeviceMemory);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkAllocateMemory() Failed for Position!.\n");
        return (vkResult);
    } else {
    ...
    // Step 10
    vkResult = vkBindBufferMemory(vkDevice, amk_snow_vertexData_random.vkBuffer, amk_snow_vertexData_random.vkDeviceMemory, 0);
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createVertexBuffer(): vkBindBufferMemory() Failed for Position!.\n");
        return (vkResult);
    } else {
    ...
    vkResult = vkMapMemory(
        vkDevice,
        amk_snow_vertexData_random.vkDeviceMemory,
        0,
        vkMemoryAllocateInfo.allocationSize,
        0,
        &data
    ...
    // Step 12
    memcpy(data, amk_points_random, sizeof(amk_points_random));

    // Step 13
    vkUnmapMemory(vkDevice, amk_snow_vertexData_random.vkDeviceMemory);
...
void calculateRandomPositionAndRandom(void) {
    ...
    // functions
    float getRandomFloat(int);

    for(int i = 0; i < POINTS_COUNT; i += 3) {
        amk_points_position[i + 0] = getRandomFloat(BOUNDING_LENGTH);
        amk_points_position[i + 1] = getRandomFloat(0) * BOUNDING_LENGTH;
        amk_points_position[i + 2] = getRandomFloat(BOUNDING_LENGTH);

        amk_points_random[i] = getRandomFloat(0);
    }

}

float getRandomFloat(int multiplier) {
    if(multiplier == 0) {
        return (float)rand() / (float)RAND_MAX;
    } else {
        return (multiplier * (((float)rand() / (float)RAND_MAX) - 0.5f));
    }
}
...
VkResult createUniformBuffer (void) {
    ...
    memset((void*)&amk_uniformData_points, 0, sizeof(UniformData));
    ...
    // Setp 4
    vkResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &amk_uniformData_points.vkBuffer);
    if(vkResult != VK_SUCCESS) {
    ...
    vkGetBufferMemoryRequirements(vkDevice, amk_uniformData_points.vkBuffer, &vkMemoryRequirements);
    ...
    //Setp 9
    vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &amk_uniformData_points.vkDeviceMemory);
    if(vkResult != VK_SUCCESS) {
    ...
    // Step 10
    vkResult = vkBindBufferMemory(vkDevice, amk_uniformData_points.vkBuffer, amk_uniformData_points.vkDeviceMemory, 0);
    if(vkResult != VK_SUCCESS) {
...
VkResult updateUniformBuffer(void) {
    ...
    vkResult = vkMapMemory(
        vkDevice,
        amk_uniformData_points.vkDeviceMemory,
        0,
    ...
    vkUnmapMemory(vkDevice, amk_uniformData_points.vkDeviceMemory);
...
VkResult createDescriptorPool(void) {
    ...
    vkDescriptorPoolCreateInfo.maxSets = 1; // we have only one descriptor set layout but we will create multiple descriptor sets from it
...
VkResult createDescriptorSet(void) {
    ...
    vkDescriptorSetAllocateInfo.descriptorPool = vkDescriptorPool; // though we have two descriptors one for mvp uniform and another for texture sampler, both are in onesame  descriptor set hence we will keep the count as 1 
    ...
    vkResult = vkAllocateDescriptorSets(vkDevice, &vkDescriptorSetAllocateInfo, &amk_vkDescriptorSet_points);
    if(vkResult != VK_SUCCESS) {
    ...
    // for mvp uniform
    vkDescriptorBufferInfo.buffer = amk_uniformData_points.vkBuffer; // this is the buffer we want to use as uniform
    ...
    vkWriteDescriptorSet_array[0].dstSet = amk_vkDescriptorSet_points; // this is the descriptor set we want to update
    ...
    vkWriteDescriptorSet_array[1].dstSet = amk_vkDescriptorSet_points; // this is the descriptor set we want to update
...
VkResult createPipeline(void) {
    ...
    vkVertexInputBindingDescription_array[1].binding = AMK_ATTRIBUTE_RANDOM; // 1st binding index for TexCoord
    ...
    // Random Attribute
    vkVertexInputAttributeDescription_array[1].binding = AMK_ATTRIBUTE_RANDOM;
    vkVertexInputAttributeDescription_array[1].location = AMK_ATTRIBUTE_RANDOM;
...
VkResult buildCommandBuffers(void) {
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++) {
        ...
        vkCmdBindVertexBuffers(
            ...
            &amk_snow_vertexData_position.vkBuffer,
            vkDeviceSize_offset_position_array
        );

        VkDeviceSize vkDeviceSize_offset_random_array[1];
        memset((void*)vkDeviceSize_offset_random_array, 0, sizeof(VkDeviceSize) * _ARRAYSIZE(vkDeviceSize_offset_random_array));

        vkCmdBindVertexBuffers(
            vkCommandBuffer_array[i], 
            AMK_ATTRIBUTE_RANDOM, 1,
            &amk_snow_vertexData_random.vkBuffer,
            vkDeviceSize_offset_random_array
        );
        ...
        vkCmdBindDescriptorSets(
            ...
            vkPipelineLayout,
            0, 1,
            &amk_vkDescriptorSet_points, // this is the descriptor set we want to bind
            0, NULL
        );

        // Here we should call vulkan drawing functions!
        vkCmdDraw(vkCommandBuffer_array[i], POINTS_COUNT, 1, 0, 0);
